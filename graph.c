...
bool exist_edge(MATRIZ_ADJ* matrix, int origin, int end) {
    ...
    if(matrix != NULL) {
        if(Directed == 1) { //Se direcionado
            if(item_get_aresta((matrix->matrix)[origin - 1][end - 1]) == 1) {
                return true;
            }
        }
        else { //Se não direcionado
            if((item_get_aresta((matrix->matrix)[origin - 1][end - 1]) == 1) || (item_get_aresta((matrix->matrix)[end - 1][origin - 1]) == 1))
                return true;
        }
        return false;
    }
...
int* get_adj_vertex(MATRIZ_ADJ* matrix, int vertex) {
    ...
    for(int i = 0; i < matrix->numero_vertices; i++) {
        lista[i] = item_get_aresta((matrix->matrix)[vertex - 1][i]);
        if(item_get_aresta((matrix->matrix)[i][vertex - 1]) == 1) //Condição necessária para grafos direcionados
            lista[i] = 1;
    }
...
bool remove_edge(MATRIZ_ADJ* matrix, int origin, int end) {
    ...
    if(matrix != NULL) {
        if(Directed == 1) { //Se direcionado
            if(item_get_aresta((matrix->matrix)[origin - 1][end - 1]) == 1)
                return(item_set_aresta_peso((matrix->matrix)[origin - 1][end - 1], 0, 0));
        }
        else { //Se não direcionado
            return((item_set_aresta_peso((matrix->matrix)[origin - 1][end - 1], 0, 0)) && (item_set_aresta_peso((matrix->matrix)[end - 1][origin - 1], 0, 0)));
        }
    }
    return false;
}
...
void print_info(MATRIZ_ADJ* matrix) {
    ...
    for(int j = 1; j <= matrix->numero_vertices; j++)
        printf("%d ", j);
    printf("\n");
...
bool add_edge(MATRIZ_ADJ* matrix, int origin, int end, int weight) {
    ...
    if(matrix != NULL && weight <= MAX_WEIGHT) {
        if(Directed == 1) { //Se direcionado
            return(item_set_aresta_peso((matrix->matrix)[origin - 1][end - 1], 1, weight));
        }
        else { //Se não direcionado
            return((item_set_aresta_peso((matrix->matrix)[origin - 1][end - 1], 1, weight)) && (item_set_aresta_peso((matrix->matrix)[end - 1][origin - 1], 1, weight)));
        }
    }
    return false;
}
...
bool remove_least_weight(MATRIZ_ADJ* matrix) {
    ...
    if(matrix != NULL) {
        ...
        for(int i = 0; i < matrix->numero_vertices; i++) {
            for(int j = 0; j < matrix-> numero_vertices; j++) {
                if((item_get_peso((matrix->matrix)[i][j]) < menor) && (exist_edge(matrix, i + 1, j + 1))) {
        ...
        if(Directed == 1) { //Se direcionado
            if(remove_edge(matrix, x+1, y+1) && menor != MAX_WEIGHT)
                return true;
        }
        else { //Se não direcionado
            if(remove_edge(matrix, x+1, y+1) && remove_edge(matrix, y+1, x+1) && menor != MAX_WEIGHT)
                return true;
        }
        return true;
    }
